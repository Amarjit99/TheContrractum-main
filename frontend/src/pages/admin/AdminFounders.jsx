...
export default function AdminFounders() {
  ...
  const handleEdit = (founder) => {
    ...
    setImagePreview(`${API}${founder.image}`);
    setIsModalOpen(true);
  };
  ...
  return (
    <AdminLayout>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 mt-2">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Founders & Directors</h1>
          <p className="text-gray-500 text-sm mt-1">Manage the profiles shown on the Founder page</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-[#f8fafc] border-b border-gray-100">
              <tr>
                ...
                <th className="text-left text-gray-500 font-semibold px-6 py-4">Type</th>
                ...
                filteredFounders.map(f => (
                  <tr key={f._id} className="hover:bg-gray-50/80 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <img src={`${API}${f.image}`} alt={f.name} className="w-10 h-10 rounded-full object-cover border border-gray-100" />
                        <span className="font-semibold text-gray-800">{f.name}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-gray-600 font-medium">{f.role}</td>
                    <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="flex justify-between items-center p-5 border-b border-gray-100">
              <h2 className="text-xl font-bold text-gray-800">{editingFounder ? 'Edit' : 'Add'} {formData.type === 'founder' ? 'Founder' : 'Founder/Director'}</h2>
